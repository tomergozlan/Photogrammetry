...
def select_100_frames(video_path, output_folder='selected_frames_lowest_SSIM', scale_factor=0.25, frame_count=100):
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        raise Exception("Error: Could not open video.")
    ...
    with tqdm(total=total_frames, desc="Analyzing Video", unit='frame') as pbar:
        while True:
            ...
            if last_frame is not None:
                # Calculate SSIM using a fixed data range for uint8 type
                score = ssim(last_frame, gray_frame, data_range=255)
                frame_scores.append((frame_index, score, frame))

            del last_frame  # Free up memory
            last_frame = gray_frame
            frame_index += 1
            pbar.update(1)
            gc.collect()  # Force garbage collection at each step
...
def select_100_similar_frames(video_path, similarity_threshold=0.8, output_folder='selected_frame_100_similar_frames', scale_factor=0.25):
    # Open the video
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        raise Exception("Error: Could not open video.")
    ...
    with tqdm(total=total_frames, desc="Analyzing Frames", unit='frame') as pbar:
        while True:
            ...
            # Convert frame to grayscale for SSIM computation
            gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY).astype(np.uint8)  # Using uint8 to save memory
            ...
            if last_frame is None:
                ...
                # Calculate SSIM with the last selected frame
                score = ssim(last_frame, gray_frame, data_range=255)  # specify data_range for uint8 images
...
# Usage
video_path = 'D:\\Photogrammetry\\Photogrammetry\\DATA\\v1.mp4'

frames1 = select_similar_frames(video_path)
print(f"Selected {len(frames1)} frames and saved to folder 'selected_frame_similar_frames'.")

frames2 = select_100_frames(video_path)
print(f"Selected {len(frames2)} frames and saved to folder 'selected_frames_lowest_SSIM'.")

frames3 = select_100_similar_frames(video_path)
print(f"Selected {len(frames3)} frames and saved to folder 'selected_frame_100_similar_frames'.")

frames4 = select_100_frames_interval_based(video_path)
print(f"Selected {len(frames4)} frames and saved to folder 'selected_frames_interval_based'.")
